...
### ADR-002: WPF UI Integration and ViewModels
* **Status**: Accepted
* **Date**: 2024-03-01
* **Context**: We need to manage UI components properly and provide effective visual notifications when background services finish execution.
* **Decision**: We implemented MaterialDesignThemes for standard visual polishing and used standard C# events in `SmartQB.Core` injected into ViewModels (e.g. `LibraryViewModel`) for inter-layer notifications. Background operations trigger UI state updates safely inside `Dispatcher.BeginInvoke`.
* **Consequences**: Prevents threading violations while providing seamless UI updates. `WebView2` correctly targets `LocalAppData` to avoid permissions faults.
